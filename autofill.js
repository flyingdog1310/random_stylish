...
let script = `document.getElementById('category').value="${cat}"; document.getElementById('title').value="${title}";  document.getElementById('description').value="${randomArr(
    ...
    placeArr
)}"; document.getElementById('note').value="${randomArr(noteArr)}"; document.getElementById('story').value="${`有一個${randomArr(
    slotAArr
)}買了它後，在${randomArr(slotBArr)}${randomArr(slotCArr)}。`}"; ${variant}`;
//以下庫存顏色隨機產生器
